...
export function buildBookmarkletJs(extras: Array<{ label: string; code: string; execution?: BookmarkletExecution }>): string {
  ...
  const injection = `setTimeout(function(){var _exbms=${extrasJson};if(_exbms.length>0){addSection('EXTRA');(function _rs(idx){if(idx>=_exbms.length)return;var bm=_exbms[idx];var _id='ex'+idx;var _lbl=bm.label.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');addRow(_id,_lbl);function _next(){setTimeout(function(){_rs(idx+1);},2000);}function _fail(tx){setRow(_id,'\\u2715','#f87171',tx||'실패');_next();}function _ok(tx){okRow(_id,tx||'완료');_next();}try{var _done=false,_seen=false,_exec=bm.execution||{},_timeoutMs=_exec.timeoutMs||12000;function _timeout(){if(!_done){_done=true;(_seen?_fail('응답 없음'):_fail('실행 오류'));}}var _timer=setTimeout(_timeout,_timeoutMs);function _restore(){doc.body.appendChild=_ac;doc.body.append=_ap;}function _finish(fn,tx){if(_done)return;_done=true;clearTimeout(_timer);_restore();fn(tx);}function _watch(n){_seen=true;if(n&&n.tagName&&n.tagName.toLowerCase()==='script'){n.addEventListener('load',function(){if(_exec.completionTopic){setRow(_id,'\\u21bb','#facc15','실행 중');}else{_finish(_ok,'로드됨');}},{once:true});n.addEventListener('error',function(){_finish(_fail,'로드 실패');},{once:true});}}if(_exec.completionTopic){var _mh=function(e){if(e.data&&e.data.topic===_exec.completionTopic){window.removeEventListener('message',_mh);_finish(_ok,'완료');}};window.addEventListener('message',_mh);}var _ac=doc.body.appendChild,_ap=doc.body.append;doc.body.appendChild=function(n){_watch(n);return _ac.call(this,n);};doc.body.append=function(){for(var _ai=0;_ai<arguments.length;_ai++)_watch(arguments[_ai]);return _ap.apply(this,arguments);};try{var _c=bm.code.replace(/^javascript:/,'');(0,eval)(_c);if(_exec.popupUrl){setRow(_id,'\\u21bb','#facc15','팝업 확인');}_seen=true;if(!_exec.completionTopic)_finish(_ok,'완료');}finally{if(!_exec.completionTopic)_restore();}}catch(_e){console.warn('[carol] extra:',bm.label,_e);_fail('실패');}})(0);}},0);`;
  const marker = "})()";
...
async function collectCore(){for(var attempt=1;attempt<=MAX_ATTEMPTS;attempt++){if(attempt>1){setRow('sv','\u21BB','#facc15','\uC7AC\uC2DC\uB3C4 '+attempt+'/'+MAX_ATTEMPTS);}var _pl=[{i:'hm',u:'/maimai-mobile/home/',o:false},{i:'pd',u:'/maimai-mobile/playerData/',o:false},{i:'rc',u:'/maimai-mobile/record/',o:false},{i:'fc',u:'/maimai-mobile/friend/userFriendCode/',o:false},{i:'tb4',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=4',o:true},{i:'tb3',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=3',o:true},{i:'tb2',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=2',o:true},{i:'tb1',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=1',o:true},{i:'tb0',u:'/maimai-mobile/record/musicGenre/search/?genre=99&diff=0',o:true},{i:'rt',u:'/maimai-mobile/home/ratingTargetMusic/',o:true}];var _r={};for(var _pi=0;_pi<_pl.length;_pi++){var _p=_pl[_pi];_r[_p.i]=await xf(_p.i,_p.u,_p.o,attempt);if(_pi<_pl.length-1)await sleep(800);}h=_r.hm;p=_r.pd;rd=_r.rc;f=_r.fc;tb4=_r.tb4;tb3=_r.tb3;tb2=_r.tb2;tb1=_r.tb1;tb0=_r.tb0;rt=_r.rt;var required={hm:h,pd:p,rc:rd,fc:f,tb4:tb4,tb3:tb3,tb2:tb2,tb1:tb1,tb0:tb0};var missing=Object.keys(required).filter(function(id){return !required[id];});if(missing.length===0)return validateCore();console.warn('[carol] missing pages attempt',attempt,missing);if(attempt<MAX_ATTEMPTS){missing.forEach(function(id){setRow(id,'\u21BB','#facc15','\uC7AC\uC2DC\uB3C4');});await sleep(500*attempt);}else{missing.forEach(function(id){failRow(id,'\uC218\uC9D1 \uC2E4\uD328');});}}return false;}
