...
import {
  calcSongRating, getConstant, levelToNumber, getJacketFile,
  getChartsUnderConstant, isNewSong,
} from "../../constants";
...
const RANK_COLOR: Record<string, number> = {
  "SSS+": 0xd97706, SSS: 0xf59e0b, "SS+": 0xfbbf24, SS: 0xfbbf24,
};
...
export function recommendCharts(clearRecords: PlayRecord[], count = 3): Recommendation[] {
  ...
  for (const r of clearRecords) {
    ...
    if (rs > topRS) { topRS = rs; topC = C; }
    if (isNewSong(r.title)) newRSs.push(rs); else oldRSs.push(rs);
  }
  if (topC <= 0) return [];
  ...
  // 후보 선별
  type Candidate = {
    title: string; kind: "ST" | "DX"; diff: string; level: number;
    currentAch: number; currentRS: number; fc: string;
  ...
  for (const chart of getChartsUnderConstant(upperBound)) {
    ...
    const validTargets = RANKS.filter(
      (rank) => rank.ach > userAch && calcSongRating(rank.ach, chart.level, fc) > floor,
    );
    if (validTargets.length === 0) continue;
    candidates.push({
      title: chart.title, kind: chart.kind, diff: chart.diff, level: chart.level,
      currentAch: userAch, currentRS: calcSongRating(userAch, chart.level, fc), fc,
      validTargets,
    });
  }
  ...
  while (chosen.length < count && pool.length > 0) {
    const pick = weightedPick(pool.map((c) => ({ item: c, weight: c.kind === "DX" ? 2 : 1 })));
    pool.splice(pool.indexOf(pick), 1);
    const target = weightedPick(pick.validTargets.map((t) => ({ item: t, weight: t.weight })));
    chosen.push({
      title: pick.title, kind: pick.kind, diff: pick.diff, level: pick.level,
      currentAch: pick.currentAch, currentRS: pick.currentRS,
      targetRank: target.name, targetAch: target.ach,
      targetRS: calcSongRating(target.ach, pick.level, pick.fc),
...
export const data = new SlashCommandBuilder()
  .setName("곡추천")
  .setDescription("레이팅 대상곡 기반으로 점수 올리기 좋은 채보 3개 추천 (생략 시 본인)")
  .addUserOption((opt) =>
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (clearRecords.length === 0) {
    await interaction.reply({ content: "기록이 없습니다. `/북마클릿`으로 먼저 동기화해주세요.", flags: MessageFlags.Ephemeral });
    return;
  }

  const recs = recommendCharts(clearRecords, 3);
  if (recs.length === 0) {
    await interaction.reply({ content: "추천할 채보를 찾지 못했습니다.", flags: MessageFlags.Ephemeral });
    return;
  }

  const embeds = recs.map((r, i) => {
    const delta = r.targetRS - r.currentRS;
    ...
    const emb = new EmbedBuilder()
      ...
      .addFields(
        { name: "채보", value: `\`${r.diff}\`  ·  상수 \`${r.level.toFixed(1)}\``, inline: true },
        { name: "목표", value: `\`${r.targetRank}\` (${r.targetAch.toFixed(1)}%+)`, inline: true },
        { name: "레이팅", value: `${r.currentRS} → **${r.targetRS}** (+${delta})`, inline: false },
        { name: "현재", value: cur, inline: true },
      );
    if (i === 0) emb.setAuthor({ name: "🎯 곡 추천 · 점수 올리기 좋은 채보" });
